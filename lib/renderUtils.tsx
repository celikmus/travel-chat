...
export function renderLandmark(location: string, url: string, info: string) {
  ...
  return <div className="text-red-600 p-1"><Image src={url} width={100} height={100} alt={`A landmark from ${location}`} style={{float: 'left', margin: '0 10px 10px 0px'}}/><span>{info}</span></div>;
}
